...
def set_log_level(level: str) -> None:
    ...
    if level.upper() not in valid_levels:
        print(f"Error: Invalid log level '{level}'. Valid levels: {', '.join(valid_levels)}")
        sys.exit(1)
    
    # Set environment variable
    os.environ["PGPT_LOG_LEVEL"] = level.upper()
    print(f"Log level set to: {level.upper()}")
    print("Note: This only affects new processes. Restart Internal Assistant for changes to take effect.")

def show_current_logs(log_dir: str = "Logs") -> None:
...
def cleanup_logs(log_dir: str = "Logs", keep_days: Optional[int] = None) -> None:
...
def show_log_tail(log_file: str = "Logs/internal_assistant.log", lines: int = 20) -> None:
...
def main():
    ...
    set_parser.add_argument('level', choices=['CRITICAL', 'ERROR', 'WARNING', 'INFO', 'DEBUG', 'NOTSET'],
                           help='Log level to set')
    
    # Show logs command
    show_parser = subparsers.add_parser('show', help='Show current log files')
    show_parser.add_argument('--dir', default='Logs', help='Log directory (default: Logs)')
    
    # Cleanup command
    cleanup_parser = subparsers.add_parser('cleanup', help='Clean up old log files')
    cleanup_parser.add_argument('--dir', default='Logs', help='Log directory (default: Logs)')
    cleanup_parser.add_argument('--keep-days', type=int, help='Keep files newer than N days')
    cleanup_parser.add_argument('--all', action='store_true', help='Remove all log files')
    
    # Tail command
    tail_parser = subparsers.add_parser('tail', help='Show last lines of log file')
    tail_parser.add_argument('--file', default='Logs/internal_assistant.log', help='Log file to show')
