...
def enforce_mode() -> Dict[str, any]:
    ...
    for package, constraint in COMPATIBILITY_CONSTRAINTS.items():
        ...
        if version is None:
            ...
            issues.append(f"ERROR {package}: {version} (required: {constraint['range']})")
            deps_ok = False
...
def check_pyproject_toml() -> Tuple[bool, str]:
    ...
    try:
        ...
        if missing_constraints:
            return False, f"pyproject.toml missing constraints: {', '.join(missing_constraints)}"
...
def fix_mode() -> Dict[str, any]:
    ...
    for package, constraint in COMPATIBILITY_CONSTRAINTS.items():
        ...
        elif not check_version_constraint(version, constraint['min'], constraint['max']):
            poetry_lock_needed = True
            poetry_install_needed = True
            fixes_applied.append(f"Will update {package} from {version} to {constraint['range']}")
    ...
    if poetry_lock_needed:
        print("\\nRunning poetry lock...")
        try:
            result = subprocess.run(["poetry", "lock"], capture_output=True, text=True, check=True)
    ...
    if poetry_install_needed:
        ...
        try:
            result = subprocess.run(["poetry", "install"], capture_output=True, text=True, check=True)
...
def display_results(results: Dict[str, any]) -> int:
    ...
    if results['mode'] == 'check':
        ...
        if results['issues']:
            ...
            print("\\nRECOMMENDATIONS:")
            print("  1. Run: python scripts/manage_compatibility.py --fix")
...
Examples:
  ...
  python manage_compatibility.py --fix
    ...
    mode_group.add_argument('--check', action='store_true', default=True,
