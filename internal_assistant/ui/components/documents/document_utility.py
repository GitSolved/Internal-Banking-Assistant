...
class DocumentUtilityBuilder:
    ...
    def __init__(
        self, ingest_service: IngestService, chat_service: ChatService | None = None
    ):
    ...
    def list_ingested_files(self) -> list[list[str]]:
        ...
        """
        logger.info(
            "🔄 [UI_REFRESH] Starting file list refresh from persistent storage"
        )
        ...
        try:
            ...
            logger.info(
                "🔄 [UI_REFRESH] Calling ingest_service.list_ingested() for fresh data"
            )
            ingested_documents = self._ingest_service.list_ingested()
            total_documents = len(ingested_documents)
            logger.info(
                "🔄 [UI_REFRESH] Retrieved %d documents from persistent storage",
                total_documents,
            )
    ...
    def format_file_list(self) -> str:
        ...
        """
        logger.info(
            "🔄 [UI_REFRESH] Starting format_file_list - reading from persistent storage"
        )
        ...
        try:
            ...
            logger.info(
                "🔄 [UI_REFRESH] Found %d unique files, getting metadata...", len(files)
            )
            ...
            ingested_documents = self._ingest_service.list_ingested()
            logger.info(
                "🔄 [UI_REFRESH] Retrieved %d documents for metadata processing",
                len(ingested_documents),
            )
