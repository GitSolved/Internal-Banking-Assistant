...
class StorageConsistencyService:
    ...
    def check_consistency(self) -> ConsistencyReport:
        ...
        logger.info(
            "🔍 [CONSISTENCY_CHECK] Starting comprehensive storage consistency check"
        )
        ...
        logger.info(
            f"📊 [CONSISTENCY_CHECK] Found {len(docstore_docs)} docs in docstore, "
        ...
        for doc_id in orphaned_docs:
            inconsistencies.append(
                StorageInconsistency(
                    ...
                    doc_id=doc_id,
                    description="Document exists in docstore but missing from vector store",
                    severity="high",
        ...
        for doc_id in orphaned_vectors:
            inconsistencies.append(
                StorageInconsistency(
                    ...
                    doc_id=doc_id,
                    description="Vector embeddings exist but document missing from docstore",
                    severity="medium",
        ...
        for doc_id in index_orphans:
            inconsistencies.append(
                StorageInconsistency(
                    ...
                    doc_id=doc_id,
                    description="Index metadata exists but document missing",
                    severity="medium",
        ...
        logger.info(
            f"✅ [CONSISTENCY_CHECK] Check completed: {len(inconsistencies)} issues found, "
    ...
    def repair_inconsistencies(
        self, report: ConsistencyReport, auto_repair: bool = False
    ) -> dict[str, int]:
        """Repair storage inconsistencies."""
        logger.info(
            f"🔧 [CONSISTENCY_REPAIR] Starting repair of {len(report.inconsistencies)} inconsistencies"
        )

        if not auto_repair and report.critical_issues:
            logger.warning(
                "⚠️ [CONSISTENCY_REPAIR] Critical issues found - manual approval required"
            )
            logger.warning(
                "⚠️ [CONSISTENCY_REPAIR] Use auto_repair=True to proceed automatically"
            )
        ...
        for inconsistency in report.inconsistencies:
            if inconsistency.severity == "critical" and not auto_repair:
                logger.info(
                    f"⏭️ [CONSISTENCY_REPAIR] Skipping critical issue (manual approval required): {inconsistency.description}"
                )
            ...
            try:
                ...
                if success:
                    ...
                    logger.info(
                        f"✅ [CONSISTENCY_REPAIR] Repaired: {inconsistency.description}"
                    )
                else:
                    repair_stats["failed"] += 1
                    logger.error(
                        f"❌ [CONSISTENCY_REPAIR] Failed to repair: {inconsistency.description}"
                    )

            except Exception as e:
                repair_stats["failed"] += 1
                logger.error(
                    f"❌ [CONSISTENCY_REPAIR] Exception repairing {inconsistency.doc_id}: {e}"
                )
    ...
    def _get_docstore_documents(self) -> set[str]:
        ...
        try:
            ...
            logger.warning(
                f"⚠️ [CONSISTENCY_CHECK] Error getting docstore documents: {e}"
            )
            return set()
    ...
    def _get_vector_store_documents(self) -> set[str]:
        ...
        try:
            ...
            if self._check_vector_collection_exists():
                ...
                return (
                    self._get_docstore_documents()
                )  # Assume consistency for existing collection
            else:
                return set()  # No collection = no vectors
        except Exception as e:
            logger.warning(
                f"⚠️ [CONSISTENCY_CHECK] Error getting vector store documents: {e}"
            )
            return set()
    ...
    def _get_index_store_documents(self) -> set[str]:
        ...
        try:
            ...
            logger.warning(
                f"⚠️ [CONSISTENCY_CHECK] Error getting index store documents: {e}"
            )
            return set()
    ...
    def _repair_single_inconsistency(self, inconsistency: StorageInconsistency) -> bool:
        ...
        logger.info(
            f"🔧 [CONSISTENCY_REPAIR] Repairing {inconsistency.type} for {inconsistency.doc_id}"
        )
        ...
        try:
            if inconsistency.type == "orphaned_document":
                ...
                logger.warning(
                    f"⚠️ [CONSISTENCY_REPAIR] Unknown inconsistency type: {inconsistency.type}"
                )
                return False

        except Exception as e:
            logger.error(
                f"❌ [CONSISTENCY_REPAIR] Error repairing {inconsistency.type}: {e}"
            )
            return False
    ...
    def _repair_orphaned_document(self, doc_id: str) -> bool:
        ...
        try:
            ...
            logger.error(
                f"❌ [REPAIR] Failed to remove orphaned document {doc_id}: {e}"
            )
            return False
    ...
    def _repair_orphaned_metadata(self, doc_id: str) -> bool:
        ...
        try:
            ...
            logger.error(
                f"❌ [REPAIR] Failed to remove orphaned metadata {doc_id}: {e}"
            )
            return False
    ...
    def _repair_missing_collection(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"✅ [REPAIR] Successfully recreated collection: {collection_name}"
            )
            return True
        except Exception as e:
    ...
    def generate_report_summary(self, report: ConsistencyReport) -> str:
        ...
        summary = []
        summary.append("📊 Storage Consistency Report")
        summary.append("─" * 40)
        ...
        if report.high_priority_issues:
            summary.append(
                f"⚠️ High Priority Issues: {len(report.high_priority_issues)}"
            )
        ...
        if report.inconsistencies:
            ...
            for issue in report.inconsistencies:
                severity_icon = (
                    ...
                    else "⚠️" if issue.severity == "high" else "ℹ️"
                )
