...
from internal_assistant.server.threat_intelligence.threat_analyzer import ThreatIndicator, SecurityRecommendation

threat_intelligence_router = APIRouter(prefix="/v1/threat-intelligence", tags=["Threat Intelligence"])


@threat_intelligence_router.get("/threats")
def get_current_threats(feed_service: RSSFeedService = Depends()) -> List[Dict[str, Any]]:
...
def get_security_recommendations(feed_service: RSSFeedService = Depends()) -> List[Dict[str, Any]]:
...
def get_critical_threats(feed_service: RSSFeedService = Depends()) -> List[Dict[str, Any]]:
...
def get_banking_specific_threats(feed_service: RSSFeedService = Depends()) -> List[Dict[str, Any]]:
    """Get threats specifically targeting banking/financial institutions."""
    threats = feed_service.analyze_threats()
    banking_threats = [
        t for t in threats 
        if any(kw in t.description.lower() for kw in ["bank", "financial", "payment", "transaction"])
    ]
...
def get_recommendations_by_priority(priority: str, feed_service: RSSFeedService = Depends()) -> List[Dict[str, Any]]:
...
def get_indicators_by_type(ioc_type: str, feed_service: RSSFeedService = Depends()) -> List[Dict[str, Any]]:
...
def get_compliance_impact_analysis(feed_service: RSSFeedService = Depends()) -> Dict[str, Any]:
