...
class FeedRequest(BaseModel):
    ...
    source: Optional[str] = Field(None, description="Filter by source (FINRA, Federal Reserve, FinCEN)")
...
class ForumRequest(BaseModel):
    ...
    search: Optional[str] = Field(None, description="Search in forum names and descriptions")
...
async def get_feeds(
    ...
    try:
        items = feed_service.get_feeds(
            source_filter=feed_request.source,
            days_filter=feed_request.days
        )
...
@inject
async def start_background_refresh(feed_service: RSSFeedService = Depends()) -> Dict[str, Any]:
...
@inject
async def get_background_status(feed_service: RSSFeedService = Depends()) -> Dict[str, Any]:
...
async def add_feed_source(
    ...
    feeds_service: RSSFeedService = Depends(lambda: global_injector.get(RSSFeedService))
) -> Dict[str, Any]:
    ...
    if success:
        ...
        raise HTTPException(status_code=400, detail=f"Failed to add feed source: {request.name}")
...
async def remove_feed_source(
    ...
    if success:
        ...
        raise HTTPException(status_code=404, detail=f"Feed source not found: {feed_name}")
...
async def refresh_forum_directory(
    forum_service: ForumDirectoryService = Depends(lambda: global_injector.get(ForumDirectoryService))
) -> Dict[str, Any]:
