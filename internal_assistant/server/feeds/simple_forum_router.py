...
simple_forum_router = APIRouter(
    prefix="/v1/feeds/forums", dependencies=[Depends(authenticated)]
)
...
async def get_forum_directory(
    forum_service: SimpleForumDirectoryService = Depends(
        lambda: global_injector.get(SimpleForumDirectoryService)
    ),
