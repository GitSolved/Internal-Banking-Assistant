...
class SimpleForumDirectoryService:
            ...
            try:
                ...
                async with self._session.get(self.TOR_TAXI_URL) as response:
                    if response.status != 200:
                        logger.warning(f"Failed to fetch tor.taxi: HTTP {response.status}")
                        return False
    ...
    def _parse_forums_section(self, html_content: str) -> List[Dict[str, str]]:
        ...
        try:
            ...
            for forum in forums:
                ...
                normalized_link = onion_link.lower().replace('https://', '').replace('http://', '').replace('www.', '')
                ...
                if normalized_link not in seen_links:
                    ...
                    logger.debug(f"Removed duplicate forum: {forum['name']} -> {forum['onion_link']}")
    ...
    def _find_forums_sections(self, soup: BeautifulSoup) -> List:
        ...
        forum_headings = soup.find_all(['h1', 'h2', 'h3', 'h4', 'h5', 'h6'], 
                                     string=re.compile(r'^Forums$', re.I))
        ...
        for heading in forum_headings:
            ...
            for sibling in heading.next_siblings:
                if hasattr(sibling, 'name') and sibling.name in ['div', 'section', 'ul', 'ol', 'table']:
                    onion_links = sibling.find_all('a', href=re.compile(r'\.onion'))
                    if onion_links:
                        forum_sections.append(sibling)
                        logger.info(f"Found forums section with {len(onion_links)} onion links")
                        break
            ...
            if parent and parent.name in ['div', 'section', 'article']:
                ...
                if onion_links:
                    ...
                    logger.info(f"Found forums section in parent with {len(onion_links)} onion links")
        
        # Strategy 2: Look for sections with "Forums" in the text and onion links
        if not forum_sections:
            logger.info("No explicit 'Forums' heading found, looking for forum sections...")
            ...
            for container in all_containers:
                ...
                if 'forum' in text and onion_links:
                    ...
                    if not any(term in text for term in ['market', 'shop', 'store', 'buy', 'sell', 'vendor']):
                        forum_sections.append(container)
                        logger.info(f"Found forum section with {len(onion_links)} onion links")
        ...
        if not forum_sections:
            ...
            for container in all_containers:
                ...
                if has_known_forums and onion_links:
                    forum_sections.append(container)
                    logger.info(f"Found section with known forums: {len(onion_links)} onion links")
    ...
    def _extract_forums_from_section(self, section) -> List[Dict[str, str]]:
        ...
        forums = []
        logger.debug(f"Extracting forums from section: {section.name if hasattr(section, 'name') else 'unknown'}")
        ...
        for link in onion_links:
            ...
            known_forum_names = ['dread', 'pitch', 'germania', 'endchan', 'xss', 'nz darknet market forum']
            forum_terms = ['forum', 'discussion', 'board', 'community', 'talk', 'chat']
            
            # Check if it's a known forum name
            is_known_forum = any(known_name in name_lower for known_name in known_forum_names)
            ...
            if not is_known_forum and (not has_forum_terms or has_marketplace):
                logger.debug(f"Skipping non-forum content: '{name}' (known: {is_known_forum}, forum: {has_forum_terms}, market: {has_marketplace})")
                continue
