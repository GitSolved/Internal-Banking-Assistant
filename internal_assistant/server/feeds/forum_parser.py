...
class ForumLink:
    ...
    def __init__(
        self, name: str, url: str, description: str = "", category: str = "General"
    ):
...
class ForumDirectoryParser:
    ...
    def parse_forum_directory(self, html_content: str) -> List[ForumLink]:
        ...
        try:
            ...
            # Final safety filter
            safe_forums = [
                forum for forum in self.parsed_forums if self._is_safe_forum(forum)
            ]

            logger.info(
                f"Extracted {len(safe_forums)} forum links from {len(forum_sections)} sections"
            )
            return safe_forums
    ...
    def _find_forum_sections(self, soup: BeautifulSoup) -> List[Tag]:
        ...
        headings = soup.find_all(
            ["h1", "h2", "h3", "h4"],
            string=re.compile(r"forum|discussion|community", re.I),
        )
        ...
        forum_containers = soup.find_all(
            ["div", "section"], {"class": re.compile(r"forum", re.I)}
        )
        forum_containers.extend(
            soup.find_all(["div", "section"], {"id": re.compile(r"forum", re.I)})
        )
    ...
    def _contains_forum_links(self, element: Tag) -> bool:
        ...
        has_forum_indicators = any(
            indicator in text for indicator in self.FORUM_INDICATORS
        )
    ...
    def _is_safe_forum(self, forum: ForumLink) -> bool:
        ...
        for keyword in self.EXCLUDED_KEYWORDS:
            if keyword in text_to_check:
                logger.debug(
                    f"Excluding forum due to keyword '{keyword}': {forum.name}"
                )
                return False
