...
from internal_assistant.server.threat_intelligence.mitre_attack_service import (
    MitreAttackService, AttackDomain, AttackTechnique, AttackTactic, ThreatGroup
)
...
async def get_techniques(
    ...
    banking_only: bool = Query(False, description="Show only banking-relevant techniques")
) -> List[Dict[str, Any]]:
...
async def get_technique_by_id(technique_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error fetching technique: {str(e)}")
...
async def get_banking_techniques() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error fetching banking techniques: {str(e)}")
...
async def get_banking_groups() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error fetching banking groups: {str(e)}")
    ...
    try:
        async with MitreAttackService() as mitre_service:
            ...
            for tactic in mitre_service._tactics_cache.values():
                if query.lower() in tactic.name.lower() or query.lower() in tactic.description.lower():
            ...
            for group in mitre_service._groups_cache.values():
                if (query.lower() in group.name.lower() or 
                    query.lower() in group.description.lower() or
                    any(query.lower() in alias.lower() for alias in group.aliases)):
                    groups.append(group)
                    ...
                    "tactics": [
                        {
                            "tactic_id": tactic.tactic_id,
                            "name": tactic.name,
                            "description": tactic.description[:200] + "..." if len(tactic.description) > 200 else tactic.description,
                            "url": tactic.url
                        }
                        for tactic in tactics
                    ],
        ...
        raise HTTPException(status_code=500, detail=f"Error searching MITRE data: {str(e)}")
...
async def refresh_mitre_data() -> Dict[str, Any]:
    ...
    try:
        async with MitreAttackService() as mitre_service:
            ...
            if success:
                return {"status": "success", "message": "MITRE ATT&CK data refreshed successfully"}
            else:
                raise HTTPException(status_code=500, detail="Failed to refresh MITRE ATT&CK data")
    except HTTPException:
        raise
    except Exception as e:
