...
def cleanup_unused_models(models_dir: str = "models", dry_run: bool = True) -> None:
    ...
    for model_dir in unused_dirs:
        ...
        if dry_run:
            print(
                f"📁 Would remove: {model_dir.name} ({dir_size / (1024*1024):.2f} MB)"
            )
        else:
            try:
...
def main():
    ...
    parser.add_argument(
        "--cleanup", action="store_true", help="Actually remove unused directories"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Show what would be done without removing",
    )
