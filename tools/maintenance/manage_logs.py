...
# Default settings
DEFAULT_LOG_DIR = "local_data/logs"
...
def show_log_status(log_dir: str = DEFAULT_LOG_DIR) -> None:
    ...
    for log_type, files in by_type.items():
        ...
        print(
            f"{log_type.capitalize()} logs: {len(files)} files, {type_size / (1024 * 1024):.2f} MB"
        )
...
def cleanup_by_session_count(
    log_files: List[Tuple[Path, float, str, int]],
    keep_count: int,
    dry_run: bool = False,
) -> int:
...
def cleanup_by_age(
    log_files: List[Tuple[Path, float, str, int]], keep_days: int, dry_run: bool = False
) -> int:
...
def cleanup_by_size(
    ...
    if dry_run:
        ...
        print(
            f"Would remove {len(files_to_remove)} files ({total_remove_mb:.2f} MB) to meet size limit:"
        )
        for file_path, mtime, log_type, size in files_to_remove:
...
def interactive_cleanup(log_dir: str = DEFAULT_LOG_DIR, keep_count: int = 7) -> None:
    ...
    if removed_count > 0:
        ...
        print(
            f"\nCleanup completed: {removed_count} files removed, {removed_mb:.2f} MB freed"
        )
    else:
...
def auto_cleanup_logs(
    ...
    print(
        f"Cleanup criteria: {keep_sessions} sessions, {keep_days} days, {max_size_mb} MB limit"
    )

    if dry_run:
        print("\nDry run - would perform the following cleanup:")
        removed_sessions = cleanup_by_session_count(
            log_files, keep_sessions, dry_run=True
        )
    ...
    if total_removed > 0:
        print(f"\nCleanup completed: {total_removed} files removed")

        # Show final status
        remaining_files = get_log_files(log_dir)
        remaining_size_mb = sum(size for _, _, _, size in remaining_files) / (
            1024 * 1024
        )
...
Examples:
  ...
  python manage_logs.py --auto --keep-sessions 10 --keep-days 14 --max-size 200
    ...
    mode_group.add_argument(
        "--status", action="store_true", help="Show log file status and information"
    )
    mode_group.add_argument(
        "--interactive",
        action="store_true",
    ...
    mode_group.add_argument(
        "--auto", action="store_true", help="Automatic cleanup using multiple criteria"
    )
