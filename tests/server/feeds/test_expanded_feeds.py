...
class TestExpandedFeeds:
    ...
    def test_source_categories(self):
        ...
        print(
            f"[PASS] All sources properly categorized into {len(expected_categories)} categories"
        )
    ...
    def test_priority_ordering(self):
        ...
        # Test US-CERT has highest priority (cyber threats)
        assert service.SOURCE_PRIORITY["US-CERT"] == 1

        print(
            f"[PASS] All sources have valid priorities (range: {min(priorities)}-{max(priorities)})"
        )
    ...
    def test_categorized_filtering(self):
        ...
        mock_items = [
            ...
            FeedItem("SEC Alert", "link3", "summary3", datetime.now(UTC), "SEC", "3"),
            FeedItem(
                "FINRA Notice",
                "link4",
                "summary4",
        ...
        # Test source filtering (not category filtering)
        fed_feeds = service.get_feeds(source_filter="Federal Reserve")
        sec_feeds = service.get_feeds(source_filter="SEC")

        # Federal Reserve feeds should only include Federal Reserve
        fed_sources = {feed["source"] for feed in fed_feeds}
        assert "Federal Reserve" in fed_sources
        assert len(fed_sources) == 1

        # SEC feeds should only include SEC
        sec_sources = {feed["source"] for feed in sec_feeds}
        assert "SEC" in sec_sources
    ...
    def test_increased_cache_size(self):
        ...
        assert (
            service.max_items == 1000
        ), f"Expected max_items=1000, got {service.max_items}"
    ...
    def test_available_sources_with_categories(self):
        ...
        mock_items = [
            ...
            FeedItem("Test2", "link2", "summary2", datetime.now(UTC), "SEC", "2"),
            FeedItem("Test3", "link3", "summary3", datetime.now(UTC), "US-CERT", "3"),
        ]
        ...
        # All configured sources should be available (including "All")
        assert len(sources) == 9  # Should have 9 sources (8 configured + "All")
