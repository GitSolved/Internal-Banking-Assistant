...
class TestRSSFeedService:
    ...
    async def test_context_manager(self, feed_service):
        ...
        assert feed_service._session.closed
    ...
    async def test_fetch_feed_no_session(self, feed_service):
        ...
        with pytest.raises(RuntimeError, match="Service must be used within async context manager"):
    ...
    async def test_refresh_feeds_success(self, feed_service, sample_rss_content):
        ...
        with patch.object(feed_service, 'fetch_feed', return_value=mock_items[:1]) as mock_fetch:
            async with feed_service:
    ...
    @pytest.mark.asyncio
    async def test_refresh_feeds_memory_limit(self, sample_rss_content):
    ...
    def test_get_feeds_no_filter(self, feed_service):
        ...
        feed_service.feeds_cache = [
            ...
            FeedItem("Title 2", "link2", "summary2", now - timedelta(hours=1), "Source 2", "2")
        ]
    ...
    def test_get_feeds_with_days_filter(self, feed_service):
        ...
        feed_service.feeds_cache = [
            ...
            FeedItem("Old", "link2", "summary2", now - timedelta(days=10), "Source", "2")
        ]
    ...
    def test_get_available_sources(self, feed_service):
        ...
        feed_service.feeds_cache = [
            FeedItem("Title 1", "link1", "summary1", datetime.now(timezone.utc), "FINRA", "1"),
            FeedItem("Title 2", "link2", "summary2", datetime.now(timezone.utc), "Federal Reserve", "2"),
            FeedItem("Title 3", "link3", "summary3", datetime.now(timezone.utc), "FINRA", "3")
        ]
        
        sources = feed_service.get_available_sources()
        
        assert "All" in sources
        assert "FINRA" in sources
        assert "Federal Reserve" in sources
        assert len(sources) == 3  # All + 2 unique sources
    
    def test_get_cache_info(self, feed_service):
        """Test getting cache information."""
        now = datetime.now(timezone.utc)
        feed_service.feeds_cache = [
            FeedItem("Title 1", "link1", "summary1", now, "FINRA", "1"),
            FeedItem("Title 2", "link2", "summary2", now, "Federal Reserve", "2")
        ]
        ...
        assert info["sources"]["FINRA"] == 1
        assert info["sources"]["Federal Reserve"] == 1
        assert info["sources"]["FinCEN"] == 0  # Not in cache