...
class TestBackgroundRefreshIsolated:
    ...
    async def test_start_and_stop_service(self):
        ...
        with patch.object(feed_service, 'refresh_feeds', new_callable=AsyncMock, return_value=True):
            with patch.object(feed_service, '__aenter__', return_value=feed_service):
                with patch.object(feed_service, '__aexit__', return_value=None):
                    bg_service = BackgroundRefreshService(feed_service, refresh_interval_minutes=1)  # 1 minute for testing
    ...
    async def test_refresh_failure_handling(self):
        ...
        with patch.object(feed_service, 'refresh_feeds', new_callable=AsyncMock, return_value=False):
            with patch.object(feed_service, '__aenter__', return_value=feed_service):
                with patch.object(feed_service, '__aexit__', return_value=None):
                    bg_service = BackgroundRefreshService(feed_service, refresh_interval_minutes=1)
